      ...
      ({
        ...
        beforeEach(() => {
          return supertestSeries([
            () => items.withAdminCookieApp.post(apiUrl).send(dummyEntries[0]),
            () => items.withAdminCookieApp.post(apiUrl).send(dummyEntries[1]),
            () =>
              items.withAdminCookieApp.get(apiUrl).then((results) => {
        ...
        it(`should DELETE the first entry from ${name} and GET new ${name} length`, (done) => {
          supertestSeries([
            () => items.withAdminCookieApp.delete(apiUrl + '/' + firstEntryId),
            () =>
              items.withAdminCookieApp.get(apiUrl).then((results) => {
        ...
        it(`should POST a new entry for ${name} and GET new ${name} length`, (done) => {
          supertestSeries([
            () => items.withAdminCookieApp.post(apiUrl).send(goodNewData),
            () =>
              items.withAdminCookieApp.get(apiUrl).then((results) => {
                strictEqual(results.body.length, originalEntriesLen + 1);
              }),
