...
describe('object', () => {
  describe('replaceObjectValues', () => {
    it('it should replace ocurrencies', () => {
      const test = { a: { b: 2, c: [{ d: 3}, ':', { e: ':', f: { g: { h: ':', i: 4}}}] } }
      const expected = { a: { b: 2, c: [{ d: 3}, 'R', { e: 'R', f: { g: { h: 'R', i: 4}}}] } }

      assert.deepStrictEqual(object.replaceObjValues(test, [[':', 'R']]), expected);
    })

    it('it should replace multiple ocurrencies', () => {
      const test = { a: { b: 2, c: [{ d: 3}, ':', { e: 'd', f: { g: { h: ':', i: 4}}}] } }
      const expected = { a: { b: 2, c: [{ d: 3}, 'R', { e: 'F', f: { g: { h: 'R', i: 4}}}] } }

      assert.deepStrictEqual(object.replaceObjValues(test, [[':', 'R'], ['d', 'F']]), expected);
    })

    it('it should return a new object if replacements were made', () => {
      const test = { a: { b: 2, c: [{ d: 3}, ':', { e: 'd', f: { g: { h: ':', i: 4}}}] } }

      assert.notStrictEqual(object.replaceObjValues(test, [[':', 'R'], ['d', 'F']]), test);
    })
